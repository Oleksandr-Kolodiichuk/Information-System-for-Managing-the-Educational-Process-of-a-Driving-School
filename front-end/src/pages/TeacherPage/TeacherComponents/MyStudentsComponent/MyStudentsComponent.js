...
const MyStudentsComponent = () => {
 ...
 const updateExamScore = async (studentId, score) => {
  try {
    ...
    const headers = getRequestHeaders();
    const response = await fetch(`http://localhost:5000/api/teacher/update-exam-score/${studentId}`, {
      method: 'PUT',
      headers: headers,
      body: JSON.stringify({ 
        exam_score: score // може бути null для видалення оцінки
      })
    });
    ...
    const updatedStudents = allStudents.map(student => 
      student.student_id === studentId 
        ? { ...student, exam_score: score } // score може бути null
        : student
    );
    
    setAllStudents(updatedStudents);
    
    // Застосовуємо фільтри до оновлених даних
    setStudents(updatedStudents.filter(student => 
      ...
      (!filters.groupName || student.group_name === filters.groupName) &&
    ...
    console.log(`Exam score ${score === null ? 'removed' : 'updated'} for student ${studentId}`);
  ...
  const generatePdf = async (studentId) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/teacher/generate-student-pdf/${studentId}`, {
        method: 'GET',
...
const generateCsv = async (studentId) => {
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/teacher/generate-student-csv/${studentId}`, {
      method: 'GET',
      headers: headers
    });
...
const generateJson = async (studentId) => {
  try {
    ...
    const response = await fetch(`http://localhost:5000/api/teacher/generate-student-json/${studentId}`, {
      method: 'GET',
      headers: headers
    });
  ...
  const filterStudents = useCallback(() => {
    ...
    if (filters.groupName) {
      filteredResults = filteredResults.filter(student => 
        student.group_name === filters.groupName
      );
    }
...
const handleScoreSubmit = (e, studentId) => {
  if (e.key === 'Enter') {
    ...
    // Якщо поле порожнє, видаляємо оцінку (встановлюємо null)
    if (value === '') {
      updateExamScore(studentId, null);
      return;
    }
    ...
    if (!isNaN(score) && score >= 0 && score <= 100) {
      ...
      // Якщо введено некоректне значення, очищаємо поле
      e.target.value = '';
...
const handleScoreBlur = (e, studentId) => {
  ...
  // Якщо поле порожнє, видаляємо оцінку
  if (value === '') {
    updateExamScore(studentId, null);
    return;
  }
  ...
  if (!isNaN(score) && score >= 0 && score <= 100) {
    ...
    // Якщо введено некоректне значення, очищаємо поле і показуємо повідомлення
    e.target.value = '';
    alert('Please enter a valid score between 0 and 100');
    updateExamScore(studentId, null);
  }
