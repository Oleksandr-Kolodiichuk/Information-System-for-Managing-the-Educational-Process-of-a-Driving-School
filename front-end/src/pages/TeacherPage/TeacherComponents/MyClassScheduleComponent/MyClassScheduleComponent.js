...
const MyClassScheduleComponent = () => {
  ...
  const fetchTeacherId = async (username) => {
    try {
      const response = await fetch(`http://localhost:5000/api/teacher/profile/${username}`, {
      ...
      setError('Error loading teacher information: ' + err.message);
      setLoading(false);
    }
  };
  ...
  const fetchLessonTypes = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/teacher/lesson_types', {
        headers: getRequestHeaders()
      });
      if (!response.ok) {
  ...
  const fetchLessonStatuses = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/teacher/lesson_statuses', {
        headers: getRequestHeaders()
      });
      if (!response.ok) {
  ...
  const fetchLessonTopics = async () => {
    try {
      const response = await fetch('http://localhost:5000/api/teacher/lesson_topics', {
        headers: getRequestHeaders()
      });
      if (!response.ok) {
  ...
  const fetchTeacherLessons = async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:5000/api/teacher/lessons/${teacherId}`, {
        headers: getRequestHeaders()
      });
      if (!response.ok) {
  ...
  const fetchGroups = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/teacher/groups/${teacherId}`, {
        headers: getRequestHeaders()
      });
      if (!response.ok) {
 ...
 const renderLessonItem = (lesson) => {
  ...
  return (
    <div 
      ...
      className={`calendar-lesson theory-lesson ${statusClass}`}
    >
      <div className="lesson-time">{formatTime(lesson.start_time)} - {formatTime(lesson.end_time)}</div>
      <div className="lesson-title">Group: {getGroupName(lesson.group_id)}</div>
      <div className="lesson-classroom">Room: {lesson.classroom_name || 'TBA'}</div>
...
const renderDetailedLessonCard = (lesson) => {
  ...
  return (
    <div key={lesson.lesson_id} className={`detailed-lesson-card theory-lesson ${statusClass}`}>
      <div className="lesson-card-header">
        <div className="lesson-time-range">
          {formatTime(lesson.start_time)} - {formatTime(lesson.end_time)}
        </div>
        <div className="lesson-type-badge theory-lesson">
      ...
      <div className="lesson-card-body">
        ...
        <div className="lesson-info-row">
          <strong>Group:</strong> {getGroupName(lesson.group_id)}
        </div>
        
        <div className="lesson-info-row">
          <strong>Classroom:</strong> {lesson.classroom_name || 'To be assigned'}
        </div>
    ...
    return (
      <div className="modal-overlay" onClick={closeDayModal}>
        <div className="day-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="modal-body" style={{ backgroundColor: 'rgb(200, 157, 217)' }}>
